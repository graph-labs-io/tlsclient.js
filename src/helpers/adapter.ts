...
export function createAdapter(_config: any) {
  ...
  return {
    destroySession: async (sessionId: string) => {
      const res = await pool.exec('destroySession', [
        JSON.stringify({ sessionId }),
      ])
      return res
    },
    destroyAll: async () => {
      const res = await pool.exec('destroyAll', [])
      return res
    },
    requestHandler: function (config: any) {
      return new Promise(async (resolve, reject) => {
        ...
        try {
          const res = await pool.exec('request', [
            JSON.stringify(requestPayload),
          ])
          const resJSON = JSON.parse(res)
          const resHeaders: Record<string, any> = {}

          await pool.exec('freeMemory', [resJSON.id])
